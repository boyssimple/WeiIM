...
@interface VCMap ()<AMapSearchDelegate,AMapLocationManagerDelegate,UITableViewDelegate,UITableViewDataSource,MAMapViewDelegate>{
...
@property (nonatomic, strong) MapInfo *current;
@end
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"位置";
    self.mapView = [[MAMapView alloc] initWithFrame:CGRectMake(0, 0, DEVICE_WIDTH, DEVICE_HEIGHT/2.0)];
...
- (void)sendLocation{
    NSLog(@"发送位置");
    UIImage *image = [self.mapView screenshot];
    if(self.datas.count > 0){
        if (!self.current) {
            self.current = [self.datas objectAtIndex:0];
        }
        if (self.current) {
            self.current.img = image;
        }
        self.mapBlock(self.current);
        [self cancel];
    }
//    VCShowPhoto *vc = [[VCShowPhoto alloc]init];
//    vc.image = image;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    self.current = [self.datas objectAtIndex:indexPath.row];
    self.current.img = [self.mapView screenshot];
}

- (UITableView*)table{
    if (!_table) {
        _table = [[UITableView alloc]initWithFrame:CGRectMake(0, self.mapView.bottom - NAV_STATUS_HEIGHT ,DEVICE_WIDTH , self.mapView.height)];
