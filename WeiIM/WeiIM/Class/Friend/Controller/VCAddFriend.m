...
- (UITextField*)tfText{
    if (!_tfText) {
        _tfText = [[UITextField alloc]initWithFrame:CGRectMake(self.lbName.right+10, 42, DEVICE_WIDTH-self.lbName.right-20, 30)];
...
- (UIButton*)btnAdd{
    if (!_btnAdd) {
        _btnAdd = [[UIButton alloc]initWithFrame:CGRectMake(10, self.lbName.bottom+30, DEVICE_WIDTH-20, 40)];
        
        [_btnAdd setBackgroundColor:APPCOLOR];
...
-(void)addAction{
    ...
    if(username.length <= 0){
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
            ...
            [[XmppCenter shareInstance]addFriendById:username];
        });
